...
const updateRoute404Data = (client: MqttClient) => {
  const pubDebug = debug(`${MAIN_TOPIC}:Mqtt:demo:pub`)

  if (process.env.NODE_ENV === 'production') {
    pubDebug(
      `This job: "${updateRoute404Data.name}" is not allowed in production.`
    )
  ...
  const route201Points = readFileSync(
    join(__dirname, '../routes/404.txt'),
    'utf8'
  )
    .split('\n')
    .map(point => point.split(',').map(coord => parseFloat(coord)))

  pubDebug(`Started job: "${updateRoute404Data.name}".`)
